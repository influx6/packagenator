...
class GuardedFile{
	...
	Future fsCheck(String path){
		...
		if(FileSystemEntity.typeSync(real) == FileSystemEntityType.NOT_FOUND) return new Future.error(new Exception('NOT FOUND!'));
	...
	Future linkTo(String path){
		...
		return link.create(this.d.path).then((_){
...
class GuardedFS{
  ...
  GuardedDirectory dir;
	...
	static Future pathExists(String path){
		if(FileSystemEntity.typeSync(path) == FileSystemEntityType.NOT_FOUND) return new Future.error(new Exception('NOT FOUND!'));
